...
func TestStore_Take(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			name:     "milli",
			index:    0,
			tokens:   []uint64{5, 6, 7, 8},
			interval: 500 * time.Millisecond,
		},
		{
			name:     "second",
			index:    100,
			tokens:   []uint64{10, 15, 20, 25},
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, tokens := range tc.tokens {
				takeCh := make(chan *result, 2*tokens)
				for i := uint64(1); i <= 2*tokens; i++ {
					go func() {
						limit, remaining, reset, ok, err := s.Take(ctx, key)
				...
				for i, result := range results {
					...
					if got, want := result.limit, tokens; got != want {
					...
					if uint64(i) < tokens {
						if got, want := result.remaining, tokens-uint64(i)-1; got != want {
